# -*- coding: utf-8 -*-
"""incrementaldumbSAT_cdevine5.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ZnQpY_RUKUEd9hgfA3w_l-8PPhlyuS8A
"""
...
# function to parse a file containing test cases
def parse_file(file_path):
    test_cases = []
    # open file for reading and loop through lines
    with open(file_path, 'r') as file:
        for line in file:
            # append numbers to list of test cases
            nums = list(map(int, line.strip().split()))
            test_cases.append(nums)
    return test_cases

# function treats the assignment as a binary number and increments it
def increment(assignment):
    # increment through assignments and flip bits to give values of T or F
...
def check(Wff, Nvars, Nclauses, Assignment):
    while True:
        satisfiable = True
        # loop through each clause in wff
...
# function to generate random wffs
def build_wff(Nvars, Nclauses, LitsPerClause):
    wff = []
    # loop to create number of clauses
    for i in range(1, Nclauses + 1):
        clause = []
        # loop to create number of literals per clause
        for j in range(1, LitsPerClause + 1):
            var = random.randint(1, Nvars)
            # randomly decide value of variable (negate or no)
...
def test_wff(wff, Nvars, Nclauses):
    # initialize assignment with 0s
    assignment = [0 for x in range(Nvars + 2)]
    # start time
    start = time.time()
    # determine if wff satisfiable
    satisfiable = check(wff, Nvars, Nclauses, assignment)
    # stop time
    end = time.time()
    # calculate time in microseconds
    execution_time = int((end - start) * 1e6)
    return [wff, assignment, satisfiable, execution_time]
...
def plot_results(sizes, times, flags):
    ...
    for size, time, is_satisfiable in zip(sizes, times, flags):
        ...
        if is_satisfiable:
            ...
            if label not in added_labels:
                plt.scatter(size, time, color='green', marker='o', label=label)
                added_labels.add(label)
            else:
                plt.scatter(size, time, color='green', marker='o')
        # if unsatisfiable, add red x on plot
        else:
            label = 'Unsatisfiable'
            if label not in added_labels:
                plt.scatter(size, time, color='red', marker='x', label=label)
                added_labels.add(label)
            else:
                plt.scatter(size, time, color='red', marker='x')
    ...
    plt.title('Time vs. Number of Variables with Incremental SAT Solver')
...
if __name__ == "__main__":
    ...
    file_path = 'data_test_cases_cdevine5.txt'
    TestCases = parse_file(file_path)
    ProbNum = 3

    resultsfile = 'output_resultsfile_cdevine5'
    tracefile = 'output_tracefile_cdevine5'
    cnffile = 'output_cnffile_cdevine5'
